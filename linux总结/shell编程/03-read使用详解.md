...
使用 **-u** 从文件描述符中读入文件, 直到其中一个文件结束 为止。
...
while [[ 1 == 1 ]]
do
read  -p "which do you like:" num
case "$num" in
        "1")
                if [ -x /script/lamp_install.sh ]
                then
                        echo "lamp installed"
                        exit 0
                else
                        echo can not install lamp
                        exit 1
                fi
                ;;
        "2")
                if [ -x /script/lnmp_install.sh ]
                then
                        echo "lnmp installed";
                        exit 0
                else
                        echo can not install lnmp
```
